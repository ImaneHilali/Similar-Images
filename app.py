...
def extract_tamura_features(image):
    ...
    directionality = graycomatrix(gray_image, [3], [0, np.pi / 4, np.pi / 2, 3 * np.pi / 4], symmetric=True, normed=True)
...
def normalize_feature(feature):
    ...
    elif isinstance(feature, list) and all(isinstance(value, (int, float, np.integer, np.floating)) for value in feature):
...
def get_channel_distances(descriptor1, descriptor2, channel):
    def get_value(descriptor, channel):
        if isinstance(descriptor, dict):
            return descriptor.get(channel, [0, 0, 0])  
...
def calculate_similarity(selected_image_data, category_images_data, seuil):
    similar_images = []
    ...
    for index, other_image_data in enumerate(category_images_data):
        ...
        for descriptor_type, weight in weights.items():
            ...
            if selected_descriptor and other_descriptor:
                feature_similarity = calculate_feature_similarity(selected_descriptor, other_descriptor, descriptor_type)
...
def calculate_similarity_route():
    ...
    similar_images = calculate_similarity(selected_image_data, category_images_data, seuil)
...
def update_weights_and_similarity():
    ...
    similar_images = calculate_similarity_with_updated_weights(selected_image_data, category_images_data, normalized_weights, seuil)

    # Return the result
    return jsonify({"weights": normalized_weights, "similar_images": similar_images})
